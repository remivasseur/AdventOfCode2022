...
public class Grid {
	...
	public Range checkWhereNoBeacon(Sensor sensor, int line) {
		...
		if (minY <= line && line <= maxY) {
			System.out.println(sensor + " -> dist :" + sensor.distanceToBeacon());
	...
	public void calculateRangeWhereNoBeacon(int lineNumber) {
		for (Sensor sensor : sensors) {
	...
	public static List<Range> correctRanges(List<Range> ranges) {
		...
		while (!rangeQueue.isEmpty()) {
			...
			Optional<Range> overlappingRange = ranges.stream().filter(
					r -> !r.equals(currentRange) && ((r.min() < currentRange.min() && r.max() >= currentRange.min())
							|| (r.min() <= currentRange.max() && r.max() > currentRange.max())))
					.findFirst();
