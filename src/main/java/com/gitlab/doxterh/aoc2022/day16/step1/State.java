...
public class State {
	...
	private int[] pressureReleased = new int[30];
	...
	public State(State previousState) {
		this(previousState.currentNode);
	...
	public boolean openValve(Node node) {
		int distance = Graph.getDistance(this.currentNode, node);
		int previousElapsedTime = this.elapsedTime;
		elapsedTime = this.elapsedTime + distance + 1;
		if (elapsedTime > 30) {
			return false;
		} else {
			for (int i=previousElapsedTime+1; i < 30; i++) {
